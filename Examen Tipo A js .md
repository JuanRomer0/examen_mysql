...
```sql
SELECT nombre , municipioid,depid,paisid
FROM clientes;
```
...
```sql
SELECT municipioid, depid, empleado
FROM empleados;
```
...
```sql
SELECT municipioid,sucursalid, empleado
FROM empleados AS total_empleados
WHERE empleado >1 ON sucursalid 
...
```sql
SELECT municipio, empleado
FROM empleados
WHERE empleado AVG(empleado) AS promedio 
WHERE empleado >= promedio;
...
FROM empleados
WHERE salario AVG (salario) AS promedio
	  salario >= promedio = Maximo
	  salario <= promedio = minimo
```
...
FROM empleados
WHERE puesto > 2 AND AVG(salario)>=1.800.000
```
